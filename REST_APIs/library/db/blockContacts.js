...
exports.blockContacts = function(mobile, contact, callback) {
	...
	MongoClient.connect('mongodb://localhost:27017/fussroll', function(err, db) {
		...
		if(err) throw err;
			...
			if(flag == 1) {
				...
				usersCollection.findOne({mobile: contact, block:{$elemMatch:{$in:[mobile, mobile_1]}}}, function(err, item) {
					...
					if(err) {
						...
						//"Mobile" is blocked by the "contact"
						db.close();
						callback(3);
					}
					else {
				...
				usersCollection.findOne({mobile: contact, block:{$elemMatch:{$in:[mobile, mobile_1, mobile_2, mobile_3]}}}, function(err, item) {
